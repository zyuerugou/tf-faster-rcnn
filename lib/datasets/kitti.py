# -*- coding: utf-8 -*-
"""
Created on Mon May  6 19:21:21 2019

KITTI datasets imdb class
...
class kitti(imdb):
  ...
  def _load_image_set_index(self):
    ...
    # self._devkit_path + /kitti_devkit/kitti/ImageSets/Main/val.txt
  ...
  def _get_default_path(self):
    ...
    """
    return os.path.join(cfg.DATA_DIR, 'kitti_devkit')
  ...
  def _load_pascal_annotation(self, index):
    """
    Load image and bounding boxes info from XML file in the Kitti
    format.
    """
    ...
    if not self.config['use_diff']:
      ...
      # user added
      #----------------------------------------
      cls_objs = [obj for obj in objs if obj.find('name').text.lower() in self._classes]
      objs = cls_objs
      print('Class: ')
      print(objs)
      #----------------------------------------
  ...
  def _get_kitti_results_file_template(self):
    # kitti_devkit/results/kitti/Main/<comp_id>_det_test_aeroplane.txt
    ...
    for cls_ind, cls in enumerate(self.classes):
      ...
      with open(filename, 'wt') as f:
        for im_ind, index in enumerate(self.image_index):
          ...
          if dets == []:
            continue
          # the kitti_devkit expects 1-based indices
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      ...
      #print(rec)
      #print(('Precision for {} = {:.4f}'.format(cls, np.mean(prec))))
    ...
    p_r_dir = os.path.join(output_dir, 'Precision-Recall_Curve.png')
    plt.savefig(p_r_dir)
    plt.show()
    #-------------------------------------------------------------------------
